...
def test_wd14_tagger_falls_back_from_tensorrt_cuda_failure_to_cpu(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    caplog: pytest.LogCaptureFixture,
    _mock_labels: list[Path],
) -> None:
    """TensorRT mode should reach CPU if the CUDA-only retry also fails."""

    attempts: list[list[str]] = []

    class _FailTensorRTAndCUDAThenCPU:
        def __init__(self, *args, **kwargs) -> None:  # noqa: D401 - signature compatibility
            providers = kwargs.get("providers", [])
            attempts.append(list(providers))
            if "TensorrtExecutionProvider" in providers or providers == ["CUDAExecutionProvider"]:
    ...
    monkeypatch.setattr(wd14_onnx.ort, "InferenceSession", _FailTensorRTAndCUDAThenCPU)
    monkeypatch.setattr(wd14_onnx, "get_available_providers", _fake_available)
    ...
    warning_messages = [record.message for record in caplog.records if record.levelno == logging.WARNING]
    assert any("TensorrtExecutionProvider unavailable" in message for message in warning_messages)
    assert any("CUDAExecutionProvider unavailable" in message for message in warning_messages)
    assert tagger._session.get_providers() == ["CPUExecutionProvider"]  # type: ignore[attr-defined]
