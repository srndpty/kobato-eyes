...
def _wait_for_writes(service: DBWritingService, expected: int, timeout: float = 2.0) -> None:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        if getattr(service, "_written", 0) >= expected:
            return
        time.sleep(0.01)
    pytest.fail(f"DBWritingService did not flush {expected} items in time (written={getattr(service, '_written', 0)})")
