...
def _wait_for_tag_rows(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.05)
    pytest.fail(
        "Timed out waiting for DBWritingService to write file_tags rows "
        f"(expected>={expected}, observed={last_count})"
    )


def _wait_for_queue_drain(service: DBWritingService, timeout: float = 5.0) -> None:
    deadline = time.monotonic() + timeout
    last_size = -1
    while time.monotonic() < deadline:
        service.raise_if_failed()
        ...
        time.sleep(0.05)
    pytest.fail(
        "Timed out waiting for DBWritingService queue to drain "
        f"(remaining={last_size})"
    )
...
def test_db_writing_standard_flow(
    ...
    try:
        ...
        _wait_for_tag_rows(str(db_path), expected=3, service=service)
    finally:
...
def test_db_writing_skip_fts(
    ...
    try:
        ...
        _wait_for_tag_rows(str(db_path), expected=1, service=service)
    finally:
        service.stop(wait_forever=True)
