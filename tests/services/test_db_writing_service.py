...
def _ensure_service_stopped(service: DBWritingService) -> None:
...
def _flush_and_stop(
    service: DBWritingService, expected: int, timeout: float = 10.0
) -> None:
    ...
    if getattr(service, "_written", 0) < expected:
        pytest.fail(
            "DBWritingService did not flush the expected number of items after stop "
            f"(expected={expected}, written={getattr(service, '_written', 0)})"
        )
...
def test_db_writing_standard_flow(
    ...
    try:
        ...
        _flush_and_stop(service, 2)
    finally:
        _ensure_service_stopped(service)
