...
class HNSWIndex:
    ...
    def __init__(self, *, space: str = "cosine") -> None:
        ...
        self._lock = threading.RLock()  # ← 追加
        self._labels: set[int] = set()  # 既存ラベルの集合
    ...
    def add(self, vectors: np.ndarray, ids: Sequence[int], *, num_threads: int = 1) -> None:
        with self._lock:
            ...
            labels = [int(x) for x in ids]
            if len(labels) != data.shape[0]:
                raise ValueError("Each vector must have a matching identifier")

            # 既存ラベルをフィルタ
            new_pairs = [(v, lid) for v, lid in zip(data, labels) if lid not in self._labels]
            if not new_pairs:
                return
            mat = np.vstack([p[0] for p in new_pairs])
            new_labels = np.ascontiguousarray([p[1] for p in new_pairs], dtype=np.int64)

            # 容量確保（余裕を持って）
            additional = mat.shape[0]
            self.ensure_capacity(self.current_count + additional + 256)

            # 追加を一度トライ（失敗時はID一覧を再取得して重複を再フィルタして1回だけ再試行）
            def _do_add(m, lbl):
                self._index.add_items(m, lbl, num_threads=int(num_threads))

            try:
                _do_add(mat, new_labels)
            except Exception:
                # ラベル一覧を同期
                try:
                    existing = set(int(x) for x in self._index.get_ids_list())
                except Exception:
                    existing = set(self._labels)
                filt = [(v, lid) for v, lid in zip(mat, new_labels.tolist()) if lid not in existing]
                if not filt:
                    return
                mat2 = np.vstack([p[0] for p in filt])
                new_labels2 = np.ascontiguousarray([p[1] for p in filt], dtype=np.int64)
                _do_add(mat2, new_labels2)
                self._labels.update(new_labels2.tolist())
                return

            self._labels.update(int(x) for x in new_labels.tolist())

    def knn_query(self, vectors: np.ndarray, k: int, *, ef: int | None = None) -> tuple[np.ndarray, np.ndarray]:
        with self._lock:
            if self._index is None:
                raise RuntimeError("Index is not initialised")
            if ef is not None:
    ...
    def load(cls, path: str | Path) -> "HNSWIndex":
        ...
        # 既存ラベルを取り込む
        try:
            instance._labels = set(int(x) for x in index.get_ids_list())
        except Exception:
            instance._labels = set()
        return instance
