...
def retag_query(
    ...
    try:
        ...
        if file_ids:
            ...
            sql = (
                "UPDATE files AS f SET tagger_sig = NULL, last_tagged_at = NULL "
                "WHERE f.id IN (" + placeholders + ")"
            )
            conn.execute(sql, tuple(file_ids))
...
class _RetagScanStage:
    ...
    def run(self, ctx: PipelineContext, emitter: ProgressEmitter) -> ScanStageResult:
        ...
        try:
            placeholders = ",".join("?" for _ in self._file_ids)
            sql = (
            ...
            rows = conn.execute(sql, tuple(self._file_ids)).fetchall()
