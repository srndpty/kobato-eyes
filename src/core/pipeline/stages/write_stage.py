...
class WriteStage:
    ...
    def run(
        ...
        emitter.emit(IndexProgress(phase=IndexPhase.FTS, done=0, total=total, message="write"), force=True)
        if total == 0:
        ...
        def _dbw_progress(kind: str, done: int, total_count: int) -> None:
            ...
            emitter.emit(IndexProgress(phase=IndexPhase.FTS, done=current_done, total=current_total, message=kind))
            try:
        ...
        try:
            ...
            emitter.emit(IndexProgress(phase=IndexPhase.FTS, done=written, total=total, message="done"), force=True)
